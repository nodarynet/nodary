...
- [ ] **R4-36** `nodary model enable|disable|restart|stage|unstage|restage` and `nodary route list|show|set` · [05 §4](../specs/05-catalog.md#4-enable-and-disable)
  ...
  - *not done:* `enable`, `disable`, `restart`, and `route list|show|set` — a deployment is still created and removed only through `model register`/`config apply`, not enabled and disabled independently
