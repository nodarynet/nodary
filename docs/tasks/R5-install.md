...
- [x] **R5-31** `nodary install` — the interactive route, asking a handful of plain questions instead of an operator having to know every verb and flag in order · [10 §1](../specs/10-cli.md#1-verbs)
  ...
  - *done:* the model step offers `--source remote` (R4-33) alongside already-staged weights — an operator with a manifest from `stage-model.sh` never has to grant this account write access to the models directory at all, since the node's own agent fetches the weights instead. Local stays the default: on the single-machine install this wizard recommends, it is one download either way, and remote would only add a manifest detour
