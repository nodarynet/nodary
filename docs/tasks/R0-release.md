...
- [ ] **R0-22** Host `nodary.net/install.sh` and `nodary.net/releases/<version>/<asset>`
  ...
  - *done in the Cloudflare account:* `nodary-releases` bucket created; the three GitHub repository secrets added; the Worker deployed with its R2 binding and all three routes bound to `nodary.net`
  - *remaining:* one DNS record — a **proxied** `AAAA nodary.net → 100::`. Until it exists the zone resolves to nothing, so the routes match nothing and the Worker is never reached. The runbook is in [`hosting/cloudflare/README.md`](../../hosting/cloudflare/README.md)
