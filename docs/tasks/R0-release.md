...
- [ ] **R0-22** Host `nodary.net/install.sh` and `nodary.net/releases/<version>/<asset>`
  ...
  - *remaining, all in the Cloudflare account:* create the `nodary-releases` bucket; add `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID` and `R2_SECRET_ACCESS_KEY` as **repository** secrets (the `build` job has no `environment:`, so an environment secret resolves to empty); add a proxied apex DNS record so the Worker routes fire at all; `npx wrangler deploy`. The runbook is in [`hosting/cloudflare/README.md`](../../hosting/cloudflare/README.md)
