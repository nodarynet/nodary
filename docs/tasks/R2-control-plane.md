...
- [ ] **R2-28** Models — list, register, show, `enable`, `disable`, `restart`, `stage`, `unstage`, delete
  - *note:* list and show are built, and registration and deletion are declarative — they go through `config apply`, which is one applier rather than a second set of writers. `enable`, `disable` and `restart` all direct a node to do something and are [R4](R4-agent.md)'s; `stage` and `unstage` are now built there too (R4-35, R4-36)
