...
- [x] **R9-21** `TestBundleVerifiesWithSha256sumAndMinisignAlone` failed once under `make check` with a short chain, and has not reproduced
  ...
  - *ruled out:* the `ts <= ?` bound is inclusive and `audit.TimeFormat` truncates to the same millisecond the column stores, so a record written in the bound's own millisecond is included; `session.now` is `time.Now()` per command, so the export's upper bound is normally later than the records it follows
  - **the assertion was racing a wall clock.** Measured: the three records land within about twenty milliseconds of each other, and the default period ends at the export command's own `time.Now()` — with the monotonic reading stripped by `.UTC()`, so the comparison is wall clock alone. A wall clock that steps backward between the writes and the export *legitimately* narrows the window and *legitimately* drops records; a VM's does when its host resyncs, which fits a failure seen once on WSL2 under a loaded parallel run and never again
  - *fixed by asserting the property instead of the timing.* The bundle test names its own window (yesterday through tomorrow), and the guard it was carrying is now two deterministic tests: a period that holds records carries exactly them, and a period that holds none **says so** in `verify.txt` rather than shipping an empty file every other check passes on. The ninety-day default is asserted against `period` directly, where there is no clock to race
  - *why it mattered more than a flaky test:* the assertion existed because a bound that silently excluded everything shipped once already. The failure mode it guards is an evidence bundle that covers a period and quietly omits records in it, which is the one artifact a customer keeps — so the answer was to guard it harder, not to loosen the number
