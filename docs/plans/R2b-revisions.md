...
## What the canonical encoder decided for us

No `json` tag on a snapshot field may carry `omitempty`, because
[`canonical`](../../internal/canonical/canonical.go) refuses one: a field that disappears when
empty makes the hash preimage depend on the value, so two snapshots describing the same
configuration would hash differently according to whether a field was written. The `toml` tags
keep it, because that side is a rendering for a human and is never hashed.

Found by the encoder rather than by review, which is [R1-01](../tasks/R1-core-audit-identity.md)
doing its job three slices later.

## Steps

- [x] Migration `0007_revision.sql`
- [x] `internal/config` — the snapshot type, its reads, canonical hashing, `Record`
- [x] TOML rendering and parsing, round-tripping through `export`/`apply`
- [x] The applier, shared by `apply` and `rollback`
- [x] `nodary config show|diff|export|apply|rollback`, and `policy apply` writing a revision
