...
| Remote administration for **every** verb. `nodary login` and `--server` are built, and the fleet, model, route and limit verbs act on a control plane over the network as the person holding the credential. The rest — users, tokens, staging, `config apply`, `usage`, `audit` — still need root on the control-plane host, and a verb that has not been converted refuses `--server` rather than quietly acting locally | [R2-44](docs/tasks/R2-control-plane.md) |
