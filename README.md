...
| Remote administration: `nodary login` stores a credential for a control plane and `--server` is being wired verb by verb, so most verbs still need root on the control-plane host and their records attribute to `root`/`local`. A verb that has not been converted refuses `--server` rather than acting on this host | [R2-44](docs/tasks/R2-control-plane.md) |
