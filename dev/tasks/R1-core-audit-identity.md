...
- [x] **R1-01** Canonical JSON encoding — deterministic key order, stable number and string forms · [07 §3](../specs/07-identity-audit.md#3-the-audit-chain)
  ...
  - *closed:* the nightly went green on 2026-09-13 (`2c55740`), the first run to include the fix — four red nights, all of which predate it. The artifact from the last red run held a second crasher the corpus did not, `2000000000010000000.0`, which is not exactly representable as a double but whose nearest double renders the identical digits: exactly the case the round-trip rule replaced the exactness test to admit. It is kept as a permanent regression input rather than merely explained
