...
| **R2** Control plane | 45 of 45 | schema, revisions, HTTP API, shared core, TLS/PKI, fleet reads (`node list`/`show`), `model register`, a derived image built over HTTP, and a failed deployment's captured log served by its own id |
| **R3** Gateway | 17 of 25 | the OpenAI surface, service keys, route allowlist, metering attributed to the deployment, node and GPU that served each request, throttling and quota, routes that carry only ready members and a `503` when none is, the data plane kept in sync automatically, and the plane itself behind a seam so a second one is a package rather than nine conditionals |
