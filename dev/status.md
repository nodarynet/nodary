...
| The backend a node is *offered*. A backend now declares the silicon it runs on and is refused by name on anything else, but `model register` still defaults `--backend vllm` on every node — including an AMD one, where no vLLM image exists and none is coming. The refusal arrives correctly; the suggestion that led there does not | [R6b](plans/R6b-the-silicon-matrix.md) |
