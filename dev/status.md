...
| A deployment's container logs. The weakest spot in the product for an operator whose deployment will not start: today the answer is on the node, and only a shell reaches it | [R2-29](tasks/R2-control-plane.md) |
...
| **R2** Control plane | 43 of 44 | schema, revisions, HTTP API, shared core, TLS/PKI, fleet reads (`node list`/`show`), `model register` |
